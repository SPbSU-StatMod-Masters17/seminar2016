...
Будем строить приближение нейронной сетью с фиксированной архитектурой[^fixed_architecture]. Функция $a$, вычисляемая сетью при зафиксированной архитектуре сети, зависит только от множества её весов: $a = a\left(\left\{w_{ij}^l\right\}\right)$ в том смысле, что в зависимости от набора весов $\left\{w_{ij}^l\right\}$ мы будем вычислять разные функции. Тогда наша задача задаче настройки весов сети, которая заключается в том, чтобы найти набор весов $$\{w_{ij}^l\} = \mathrm{arg\,min}_{\{w_{ij}^l\}} J\left(f, a\left(\left\{w_{ij}^l\right\}\right)\right),$$ где $J$ -- функция, измеряющая расстояние между $f$ и $a\left(\left\{w_{ij}^l\right\}\right)$.
