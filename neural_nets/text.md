...
Эта модель естественным образом расширяется: $\varphi$ может быть не ступенчатой функцией, а, например, сигмоидальной ($\sigma(z) = 1 / \left(1 + e^{-z}\right)$), просто линейной ($\mathrm{id}(z) = z$) или обрезанной линейной ($\mathrm{ReLU}(z) = \max\left\{0, z\right\}$)
